document.addEventListener('DOMContentLoaded', () => {
    ...
    directoryTree.addEventListener('click', (event) => {
        ...
        if (folderTarget) {
            ...
            addFolderButton.disabled = !directoryNameInput.value.trim();
            addFolderButton.classList.toggle('active', directoryNameInput.value.trim());
            addFileButton.disabled = false;
            addFileButton.classList.add('active');
            deleteButton.disabled = false;
            deleteButton.classList.add('active');
            ...
            document.querySelector('.photos__img').src = '../public/images/noimage.jpeg';
            ...
            const parentFolderPath = fileTarget.closest('.directory__item').querySelector('.directory__folder')?.dataset.path || '';
            const folderPath = fileTarget.closest('.directory__item').querySelector('.directory__folder').dataset.path;
            const fileName = fileTarget.textContent.trim();
            selectedFilePath = `${parentFolderPath}${fileName}`;
            selectedPathLabel.textContent = `Выбрано: ${folderPath}${fileName}`;

            const isImage = /\.(jpg|jpeg|png|gif)$/i.test(fileName);
            if (isImage) {
                document.querySelector('.photos__img').src = `../uploads/${fileName}`;
                document.querySelector('.photos__img').style.display = 'block';
            } else {
                document.querySelector('.photos__img').src = '../public/images/noimage.jpeg';
            }
            ...
            downloadButton.classList.toggle('active', isImage);
        }
    });
